...
def generate_train_test_splits(data_path, number_splits, min_interactions_user, data_type, train_pct):
    ...
    test_users = sample(list(range(100)), int(100 * (1-train_pct))) # len(users_gtmin)
    test_df = df[df.user_id.isin(test_users)]
    train_val_df = df[~df.user_id.isin(test_users)]
    assert not (set(train_val_df.user_id.unique()) & set(test_df.user_id.unique()))

    train_val_splits = []
    for i in range(number_splits):
        train_users = sample(list(range(100)), int(100 * train_pct)) # len(users_gtmin)
        # test_users = list(set(users_gtmin) - set(train_users))
        train_df, val_df = train_val_df[train_val_df.user_id.isin(train_users)], train_val_df[~train_val_df.user_id.isin(train_users)]
        train_val_splits.append((train_df, val_df))
        assert not (set(train_df.user_id.unique()) & set(val_df.user_id.unique()))


    return train_val_splits, (id_mapping_dict, user_mapping_dict), (total_items, total_users), test_df
